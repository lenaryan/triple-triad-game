...
const Biography = ({ id, onBackClick }) => {
    ...
    return (
        <section className={s.root}>
            <Container>
                <Button className={s.centeredBlock} blackbtn onClick={handleClick}>Go Back</Button>
                {
                    BIO[id].map(item => {
                        switch(item.type) {
                            case 'paragraph': return (
                                <Text element='p'>{item.text}</Text>
                            );
                            case 'img': return (
                                <img className={s.centeredBlock} src={item.src} alt=" "/>
                            );
                            case 'h1':
                            case 'h2':
                            case 'h3':
                            ...
                            case 'h5':
                            case 'h6': return (
                                <Heading className={s.marginTop} level={+item.type.charAt(1)}>{item.text}</Heading>
                            );
                            default: return (
                                <Text element={item.type}>{item.text}</Text>
                            )
                        }
                    })
                }
